...
The system SHALL launch a StarCraft II game instance and connect a Protoss bot to it using the python-sc2 `run_game` interface.

#### Scenario: Game starts successfully
- **WHEN** the user runs the bot launch script
...
- **WHEN** the specified map is not found in the Maps directory
- **THEN** the system SHALL report a clear error message indicating which map is missing and where to download it

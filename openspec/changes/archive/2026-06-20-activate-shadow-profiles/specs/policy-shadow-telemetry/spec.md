...
- **WHEN** policy mode is `ml_shadow` and shadow predictions exist
- **THEN** the system SHALL NOT execute any shadow prediction instead of the heuristic action
...
#### Scenario: Shadow predictions array recorded
- **WHEN** policy mode is `ml_shadow`
- **THEN** the decision record SHALL include `shadow_predictions` as a JSON array. Each element SHALL contain `profile` (string) and `recommended_action` (Action shape with `type`, `target`, `score`).

#### Scenario: Shadow predictions empty when no shadows configured
- **WHEN** policy mode is `ml_shadow` and no shadow profiles are configured
- **THEN** the decision record SHALL include `shadow_predictions` as an empty JSON array `[]`

#### Scenario: Shadow predictions for each configured profile
- **WHEN** policy mode is `ml_shadow` and N shadow profiles are configured
- **THEN** the `shadow_predictions` array SHALL contain exactly N entries, one per profile
...
#### Scenario: Shadow profiles included in policy metadata
- **WHEN** a match runs in `ml_shadow` mode with shadow profiles configured
- **THEN** the report `policy` object SHALL include a `shadow_profiles` array listing the configured shadow profile names
...
### Requirement: Shadow prediction singular field
**Reason**: Replaced by `shadow_predictions` array to support multiple shadow profiles running simultaneously.
**Migration**: Consumers reading `shadow_prediction` (singular) from `decisions.jsonl` must be updated to read `shadow_predictions` (plural, array). Each array element has the shape `{"profile": "<name>", "recommended_action": {"type": "...", "target": "...", "score": <float>}}`.
